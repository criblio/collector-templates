...
- `<ServiceNow sysevent URL|Full ServiceNow Table API URL for sysevent, e.g. https://INSTANCE.service-now.com/api/now/table/sysevent>`
- `<ServiceNow Username|ServiceNow account used for API access>`
- `<ServiceNow Password|Password or token for the ServiceNow user>`

The collector uses a relative time range with `earliest` set to `-7d` and tracks `latestTime` state based on `sys_created_on`.

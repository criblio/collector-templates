...
Splunk or OCSF, serializing to `_raw`) belongs in a post-processing pipeline — see below.
...
fields to the names used by the Cribl **Akamai SIEM Pack** and its OCSF mapping (`Rule`→`id`,
...
5. (Optional) Enable the `json_unroll` function to emit **one event per matched WAF rule** instead of
   one per request. OCSF Detection Finding expects this, since each rule is a separate finding — but
   it multiplies your event count, and therefore your Splunk licence volume.
